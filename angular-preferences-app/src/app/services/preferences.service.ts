...
export class PreferencesService {
    ...
    private loadPreferences(): void {
        try {
            ...
            if (savedPrefs) {
                ...
                // Apply loaded preferences to services (with fallbacks)
                ...
                this.shapeService.setShape(prefs.borderRadiusId ?? this.shapeService.defaultShape.id);
            }
        } catch (e) {
    ...
    public resetToDefaults(): void {
        ...
        this.typographyService.setFontSize(this.typographyService.defaultFontSize.id);
    ...
    // 1. Theme Color (e.g., .blue-theme)
    private updateColorThemeClass = effect(() => {
        if (!isPlatformBrowser(this.platformId)) return;
        const theme = this.themeService.currentTheme();
        
        const allThemeClasses = this.themeService.getThemes().map((t) => `${t.id}-theme`);
    ...
    private updateDensityClass = effect(() => {
        ...
        // Clean up all possible density classes
    ...
    private updateShapeVariable = effect(() => {
        ...
        this.document.documentElement.style.setProperty('--app-border-radius', `${shape.pixelValue}px`);
    });
}